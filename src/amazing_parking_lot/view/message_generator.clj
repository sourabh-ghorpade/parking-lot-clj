...
(defn generate-message [response-code action]
  (condp = response-code
    ...
    (event/status-codes :slot-numbers-for-cars-with-colour) (clojure.string/join ", " (:query-result action))
    (event/status-codes :slot-number-for-car-with-registration-number) (str (:query-result action))
