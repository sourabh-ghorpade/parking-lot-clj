...
(deftest validate-test
  ...
  (testing "when the option is invalid"
    (testing "it returns the same invalid option"
      (let [invalid-option (option-builder/create-invalid-option "some error")
            validated-option (validate invalid-option)]
        (is (false? (:valid? validated-option)))))))
